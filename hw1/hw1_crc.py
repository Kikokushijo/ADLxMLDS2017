...
def set_model():
    model = Sequential()
    model.add(Conv1D(filters=32, kernel_size=11, padding='same', activation='relu', input_shape=(800, 108), kernel_initializer='he_normal'))
    model.add(Conv1D(filters=64, kernel_size=9, padding='same', activation='relu', kernel_initializer='he_normal'))
    model.add(Conv1D(filters=128, kernel_size=7, padding='same', activation='relu', kernel_initializer='he_normal'))
    model.add(Conv1D(filters=256, kernel_size=5, padding='same', activation='relu', kernel_initializer='he_normal'))
...
if __name__ == "__main__":
    ...
    model = set_model()
    filepath = "models/very_deep_RNN_model.h5"
